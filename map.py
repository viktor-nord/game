...
class Map:
  ...
  def add_world_border(self):
    # some bug here with "down_border"
    w, h = (self.tmxdata.width * size) / \
      2, (self.tmxdata.height * size) / 2
    colliders = {
    ...
    for layer_index, layer in enumerate(self.tmxdata.layers):
      ...
      if isinstance(layer, pytmx.TiledTileLayer):
        for tile in layer.tiles():
          properties = self.tmxdata.get_tile_properties(
            tile[0], tile[1], layer_index
          )
          properties["frame_images"] = [self.tmxdata.get_tile_image_by_gid(
            frame.gid
...
class Collider:
  ...
  def get_dots(self, points):
    ...
    vartical_down = (self.x, self.y) in points and (
      self.x + size, self.y + size) in points
